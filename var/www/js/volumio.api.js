...
function backendRequest() {
    $.ajax({
        type : 'GET',
        url : '_player_engine.php?state=' + (GUI.MpdState['state'] || 'stop'),
        async : true,
        cache : false,
        success : function(data) {
			GUI.MpdState = data;
...
function updateGUI(objectInputState) {
  ...
  var elapsed = parseInt(objectInputState['elapsed'], 10);
  var progressed = !isNaN(elapsed) && elapsed > GUI.playPendingElapsed;


  if (GUI.playPending && objectInputState['state'] === 'play' && progressed) {
    GUI.playPending = false;
  }

  if (objectInputState['state'] === 'play') {
    $('#play i').removeClass().addClass('fa fa-pause');
    $('#play').addClass('btn-primary').removeClass('active pending');

  } else if (GUI.playPending) {

    $('#play i').removeClass().addClass('fa fa-spinner fa-spin');
    $('#play').addClass('pending').removeClass('btn-primary active');

  } else if (objectInputState['state'] === 'pause') {
    $('#playlist-position').html('Not playing');
    $('#play i').removeClass().addClass('fa fa-play');
    $('#play').removeClass('btn-primary active pending');
  ...
  if (GUI.playPending) {
    var total = parseInt(objectInputState['time'], 10);
    var timeout = (!isNaN(total) && total > 0) ? 5000 : 15000;

    if (objectInputState['state'] === 'play' && progressed) {
      GUI.playPending = false;
    } else if (Date.now() - GUI.playPendingAt > timeout) {
      GUI.playPending = false;
      sendCmd('stop');
      $('#play i').removeClass('fa fa-pause').addClass('fa fa-play');
      $('#play').removeClass('btn-primary active');
      if ($.pnotify) $.pnotify({ title: 'Playback', text: 'Playback failed', type: 'error' });
    }
  }
...
//	var fileinfo = (objectInputState['audio_sample_depth'] && objectInputState['audio_sample_rate']) ? (' - ' + objectInputState['audio_sample_depth'] + ' bit - ' + objectInputState['audio_sample_rate'] +' Hz ') :;
	...
	$('#playlist-position')[ (Number.isFinite(parseInt(objectInputState['song'],10)) && Number.isFinite(parseInt(objectInputState['playlistlength'],10)) && parseInt(objectInputState['playlistlength'],10) > 0) ? 'show' : 'hide' ]().html('Playlist position ' + (parseInt(objectInputState['song'],10)+1) + '/' + parseInt(objectInputState['playlistlength'],10));
	$('.playlist li').removeClass('active');
