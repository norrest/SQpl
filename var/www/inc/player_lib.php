...
// v3
function readMpdResponse($sock, $timeoutSeconds = 2) {
    $output = "";
    if (!$sock) return $output;
    ...
    while (!feof($sock)) {
        ...
        if (strncmp(MPD_RESPONSE_OK, $response, strlen(MPD_RESPONSE_OK)) == 0) break;
...
function monitorMpdState($mpd, $waitSeconds = 10) {

    if (!$mpd) return array();

    $events = 'player mixer options playlist';


    @fwrite($mpd, "idle $events\n");
    ...
    $n = @stream_select($r, $w, $e, (int)$waitSeconds);
    ...
    if ($n === false || $n === 0) {
        ...
        $r2 = array($mpd);
        @stream_select($r2, $w, $e, 1);
        @readMpdResponse($mpd, 1);


        return _parseStatusResponse(MpdStatus($mpd));
    }


    @readMpdResponse($mpd, 2);
...
function _parseStatusResponse($resp) {
	if ( is_null($resp) ) {
		...
		$plistArray['audio_sample_rate'] = ($audio_format[0]);
		...
		switch ($audio_format[1]) {
			...
			case '1':
			case '2':
			$plistArray['audio_sample_depth'] = 1;
			break;
		}
		...
		// format "audio_channels" string
			...
			// else $plistArray['audio_channels'] = "Multichannel"
			
	if ($audio_format[2] == "2") {
	 	$plistArray['audio_channels'] = "Stereo";
	} else if ($audio_format[2] == "1" || $audio_format[2] == '*') {
	 	$plistArray['audio_channels'] = 'Stereo';
	} else if ($audio_format[2] > 2) {
	 	$plistArray['audio_channels'] = "Multichannel";
	}	
	   else  $plistArray['audio_channels'] = '&nbsp;'; 
	}
