...
<head>
<meta charset="utf-8">
<title>StereoQ AirPlay Update</title>
...
<div class="wrap">
  <div class="panel">
    <div class="header">
      <h1 class="title">StereoQ AirPlay Update</h1>
      <div class="sub">
        This will update Shairport Sync. Internet access is required.
      </div>
    </div>

    <div class="notice">
      You are about to update the AirPlay component of the player.<br>
      Please keep this page open until the process finishes.<br>
      A manual reboot may be required after the update completes.
    </div>

    <form method="POST" class="actions">
      <button type="submit" name="update" value="1">Update AirPlay</button>
      <div class="hint">Do not refresh this page while the update is running.</div>
    </form>
  </div>
</div>
...
<div class="wrap">
  <div class="panel">
    <div class="header">
      <h1 class="title">StereoQ AirPlay Update</h1>
      <div class="sub">
        Shairport Sync update is running. Output will appear below.
      </div>
    </div>
...
echo "[+] Shairport Sync update started: ".date('H:i:s')."\n\n";
echo str_repeat(" ", 4096)."\n";
flush();

$cmd = 'sudo -n /bin/bash /sbin/shairport-sync-update 2>&1';
$h = popen($cmd, 'r');
...
if ($h) {
  ...
  echo "[!] Cannot start Shairport Sync update\n";
  $code = 1;
}
?></div>
